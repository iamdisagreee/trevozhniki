...
class MessageService:
    ...
    async def get_all_chats(
        ...
        chats = await self.msg_repo.get_all_chats(user_id=user_id)
        chats_json = {'chats':
            [
                {
                    'id': chat.id,
    ...
    async def get_limit_chats(
        ...
        chats_limit = await self.msg_repo.get_limit_chats(user_id=user_id)
    ...
    async def reload_last_chat_id(
            self,
            user_id: int
    ):
        await self.msg_repo.update_last_chat_id(user_id=user_id)
        return JSONResponse(
            content={'detail': 'Id successfully reloaded!'}
        )
